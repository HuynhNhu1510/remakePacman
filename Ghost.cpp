...
Ghost::Ghost(unsigned char i_id) :
	id(i_id){
//vi ko biet ghi gi nen ghi dai vo cho vui :)
}
bool Ghost::pacman_collision(const Position& i_pacman_position) {
	if (position.x > i_pacman_position.x - Cell_Size && position.x < Cell_Size + i_pacman_position.y) {
...
void Ghost::draw(bool i_flash, sf::RenderWindow& i_window)
{
	...
	sf::Texture texture;
	texture.loadFromFile("Resources/Ghost16" + std::to_string(Cell_Size) + ".png");
	...
	if (0 == frightened_mode) {
		...
		//scare pacman and run
		else if (1 == frightened_mode){
			body.setColor(sf::Color(36, 36, 255));
			//the position face the same the position ghost
			...
			//only draw eyes pacman eat the body
	...
	void Ghost::update(unsigned char i_level, std::array<std::array<Cell, Map_Height>, Map_Width>&i_map, Ghost & i_ghost_0, Pacman & i_pacman)
	{
		...
		{
			frightend_speed_timer = Ghost_Frightened_Speed;
